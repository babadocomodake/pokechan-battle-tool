...
// きれあじ(Sharpness)用: データに slicing フラグが無いため切断技をホワイトリスト化
const SLICING_MOVES = new Set([
  "Air Cutter", "Air Slash", "Aqua Cutter", "Behemoth Blade", "Bitter Blade", "Ceaseless Edge",
...
function effectivePower(move, cond) {
  ...
  if ((DOUBLE_IF_HIT_MOVES.has(name) || DOUBLE_IF_AFTER_MOVES.has(name)) && cond.movedAfter) p *= 2;
  return p;
}
...
function damageTab(preset) {
  ...
  const moveSel = el("select", { class: "move-select", onchange: () => { renderConds(); render(); } });
  ...
  // ②-C/A2 「わざ横」条件入力。選んだ技・特性に応じて、必要な入力だけを1か所にまとめて表示する。
  ...
  function renderConds() {
    ...
    const needHp = HP_LOWER_MOVES.has(mv) || HP_HIGHER_MOVES.has(mv);
    const needFainted = FAINTED_MOVES.has(mv) || ab === "Supreme Overlord";
    const needAfter = DOUBLE_IF_HIT_MOVES.has(mv) || DOUBLE_IF_AFTER_MOVES.has(mv) || ab === "Analytic";
    if (needHp) rows.push(fieldRow("自分の残りHP%（この技の威力に反映）", atkHpPctInput));
    if (needFainted) rows.push(fieldRow("倒れた味方の数（0〜5）", faintedInput));
    if (needAfter) rows.push(el("label", { class: "toggle" }, [afterCb, " 相手より後に動いた（後攻）＝威力/威力補正2倍・アナライズ1.3倍"]));
    if (ab === "Rivalry") rows.push(fieldRow("とうそうしんの性別関係", rivalrySel));
    if (NEEDS_MANUAL_POWER.has(mv)) rows.push(fieldRow("威力を手動入力（体重/素早さ依存のため）", manualPowerInput));
    // 自動反映系はヒントだけ表示（入力不要）
    if (mv === "Acrobatics") rows.push(el("p", { class: "hint" }, "自動: 持ち物なしで威力2倍"));
  ...
  function render() {
    ...
    renderConds();        // 可変威力わざ/条件付き特性の条件入力を選択に合わせて表示
